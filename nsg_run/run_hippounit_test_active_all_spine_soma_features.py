...
def run_test(ind_and_params):
    ...
    # user function path
    """Edit the line below"""
    user_function_path = './usr_fun_model_full_rework7_for_allspine_R_tuft_more.txt'
    ...
    """Edit the line below"""
    model.hocpath = "./model/load_model_na_inhomo_minimal_model_full_soma_all_spine_true_diam_active_spine_KA_fact_50Ra.hoc" 
    ...
    """Edit the line below"""
    base_directory = './Active_all_spine/'

    # Load target data
    """Edit the line below"""
    with open('./target_features/CA1_valid_data_uniform.json') as f:
        observation = json.load(f, object_pairs_hook=collections.OrderedDict)
        
    import pkg_resources
    stim_file = pkg_resources.resource_filename("hippounit", "tests/stimuli/somafeat_stim/stim_CA1_valid_data.json")
    with open(stim_file, 'r') as f:
        config = json.load(f, object_pairs_hook=collections.OrderedDict)

    # Instantiate the test class
    test = tests.SomaticFeaturesTest(observation = observation, config=config, save_all = True, force_run=False, show_plot = True, base_directory = base_directory)
