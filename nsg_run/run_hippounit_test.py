...
def print_params(ind_and_params):

    import sys
    import matplotlib.pyplot as plt

    ind, parameters = ind_and_params
    sys.__stdout__.write(str(ind) + str(parameters))

    plt.plot(parameters)
    plt.savefig('./figure_' + str(ind))
...
def run_test(ind_and_params):
    ...
    from hippounit.utils import ModelLoader_Modellke, ModelLoader_synapse_on_Existing_Spine
    ...
    # user function path
    """Edit the line below"""
    user_function_path = './usr_fun_model_full_jav.txt'

    #Load cell model
    model = ModelLoader_Modellke(mod_files_path = mod_files_path, user_function_path = user_function_path)  # Change this to existing spines later

    # path to hoc file
    # the model must not display any GUI!!
    """Edit the line below"""
    model.hocpath = "./model/load_model_na_inhomo_minimal_model_full_soma_f_fact_true_diam_passive_spine_KA_fact.hoc"
    ...
    """Edit the line below"""
    base_directory = './ipyparallel_2_results/'
    ...
    test = tests.ObliqueIntegrationTest(observation = observation, save_all = True, force_run_synapse=False, force_run_bin_search=False, show_plot = False, base_directory = base_directory)
