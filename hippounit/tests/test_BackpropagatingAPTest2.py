...
class BackpropagatingAPTest2(Test):
    """
    ...
    def run_cclamp_on_soma(self, model, amp, delay, dur, section_stim, loc_stim, section_rec, loc_rec):
        ...
        if self.base_directory:
            self.path_temp_data = self.base_directory + 'temp_data/' + 'backpropagating_AP2/' + model.name + '/'
        else:
            self.path_temp_data = model.base_directory + 'temp_data/' + 'backpropagating_AP2/'
    ...
    def find_current_amp(self, model, delay, dur, section_stim, loc_stim, section_rec, loc_rec):
        ...
        if self.base_directory:
            self.path_figs = self.base_directory + 'figs/' + 'backpropagating_AP2/' + model.name + '/'
        else:
            self.path_figs = model.base_directory + 'figs/' + 'backpropagating_AP2/'
    ...
    def extract_somatic_spiking_features(self, traces, delay, duration):
        ...
        traces_results = efel.getFeatureValues(traces_for_efel, ['inv_first_ISI','AP_begin_time', 'doublet_ISI', 'AP1_amp', 'APlast_amp'])
    ...
    def plot_features(self, model, features, actual_distances, traces_results):
        ...
        for key, value in features.items():
            ...
            for k, v in features[key].items() :
                ...
                model_AP1_amps = numpy.append(model_AP1_amps, features[key][k]['AP1_amp']/traces_results[0]['AP1_amp'])
                model_APlast_amps = numpy.append(model_APlast_amps, features[key][k]['APlast_amp']/traces_results[0]['APlast_amp'])
        ...
        plt.ylabel('AP1_amp dendrite / AP1_amp_soma')
        lgd = plt.legend(bbox_to_anchor=(1.0, 1.0), loc = 'upper left')
        if self.save_all:
        ...
        plt.xlabel('Distance from soma (um)')
        plt.ylabel('APlast_amp_dendrite / APlast_amp_soma')
        lgd = plt.legend(bbox_to_anchor=(1.0, 1.0), loc = 'upper left')
        if self.save_all:
    ...
    def generate_prediction(self, model, verbose=False):
        ...
        if self.base_directory:
            self.path_results = self.base_directory + 'results/' + 'backpropagating_AP2/' + model.name + '/'
        else:
            self.path_results = model.base_directory + 'results/' + 'backpropagating_AP2/'
        ...
        for key in features:
            ...
            for k in features[key]:
                AP1_amps = numpy.append(AP1_amps, features[key][k]['AP1_amp']/traces_results[0]['AP1_amp'] )
            prediction['model_AP1_amp_at_'+str(key)+'um'] = {}
            prediction['model_AP1_amp_at_'+str(key)+'um']['mean'] = float(numpy.mean(AP1_amps))
            prediction['model_AP1_amp_at_'+str(key)+'um']['std'] = float(numpy.std(AP1_amps))

        for key in features:
            AP1_amps = numpy.array([])
            APlast_amps = numpy.array([])
            for k in features[key]:
                APlast_amps = numpy.append(APlast_amps, features[key][k]['APlast_amp']/traces_results[0]['APlast_amp'] )
            prediction['model_APlast_amp_at_'+str(key)+'um'] = {}
            prediction['model_APlast_amp_at_'+str(key)+'um']['mean'] = float(numpy.mean(APlast_amps))
            prediction['model_APlast_amp_at_'+str(key)+'um']['std'] = float(numpy.std(APlast_amps))
        ...
        self.plot_features(model, features, actual_distances, traces_results)
