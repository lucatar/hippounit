...
class PathwayInteraction(Test):
    ...
    def generate_no_input_traces(self, model, recording_loc):
        ...
        if self.force_run or (os.path.isfile(file_name_no_input) is False):
            model.initialise()
            t, v, v_dend,v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation([], recording_loc, 1600, 'SC') #ide csak talalomra beirtam hogy SC mert igazabol mindegy
    ...
    def theta_pathway_stimulus(self, model, SC_weight, PP_weight, SC_dend_loc, PP_dend_loc, recording_loc, stimuli_params, tstop, depol_amp, pathway, save_traces):
        ...
        if (self.force_run or (os.path.isfile(file_name) is False)) and save_traces:
            ...
            if pathway == 'SC':
                ...
                t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation(SC_dend_loc, recording_loc, tstop, pathway)
                print('roseapple2')
                '''
                plt.figure()
                plt.plot(t,v)
                ...
                t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation(PP_dend_loc, recording_loc, tstop, pathway)
                print('roseapple2')
                '''
                plt.figure()
                plt.plot(t,v)
                ...
                pathway2 =['SC', 'PP']
                print('grapes')
                for pathway3 in pathway2:
                   print('current pathway:', pathway3)
                   if pathway3 == 'SC':
                       t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation(SC_dend_loc, recording_loc, tstop, pathway3) #SC_dend_loc + PP_dend_loc
                   if pathway3 == 'PP':
                       t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation(PP_dend_loc, recording_loc, tstop, pathway3) #SC_dend_loc + PP_dend_loc
                print('grapes2')
                '''
                plt.figure()
                plt.plot(t,v)
            ...
            # plt.show()
                ...
                t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation([], recording_loc, tstop, pathway)
                ...
                model.activate_theta_stimuli(PP_dend_loc, PP_weight, 'PP', interval_bw_trains, interval_bw_stimuli_in_train, num_trains, num_stimuli_in_train)
                model.activate_current_stimuli(depol_amp, model.start, num_stimuli_in_train * interval_bw_stimuli_in_train, num_trains, interval_bw_trains, rec_ndend, xloc)
                t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation(PP_dend_loc, recording_loc, tstop, 'PP')
                print('maracuja')
                ...
                model.activate_theta_stimuli(SC_dend_loc, SC_weight, 'SC', interval_bw_trains, interval_bw_stimuli_in_train, num_trains, num_stimuli_in_train)
                model.activate_current_stimuli(depol_amp, model.start, num_stimuli_in_train * interval_bw_stimuli_in_train, num_trains, interval_bw_trains, rec_ndend, xloc)
                t, v, v_dend, v_stim_locs, v_stim_locs_spine, v_stim_locs_spine_n1, v_stim_locs_spine_n2 = model.run_simulation(SC_dend_loc, recording_loc, tstop, 'SC')

            traces = {pathway: {'t' : t, 'v_soma' : v, 'v_dend' : v_dend, 'v_stim_locs' : v_stim_locs, 'v_stim_locs_spine': v_stim_locs_spine, 'v_stim_locs_spine_n1': v_stim_locs_spine_n1, 'v_stim_locs_spine_n2': v_stim_locs_spine_n2}} 
